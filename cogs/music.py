...
class Music(commands.Cog):
    ...
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('Nothing being played at the moment.', delete_after=20)
        ...
        if 0 <= volume <= 101:
            ...
            return await ctx.send(f'**`{ctx.author}`**: Set the volume to **{volume}%**')
    ...
    @commands.command(name='skip', aliases=['fs','next','s'])
    ...
    async def _remove(self, ctx: commands.Context, index: int):
        ...
        if len(ctx.voice_state.songs) <index:
            return await ctx.send(f'Failed to remove song: Tracks remaining - **{len(ctx.voice_state.songs)}**')

        embed=discord.Embed(title='Removed',timestamp=datetime.datetime.utcnow(), description='```css\n{0.source.title}\n```'.format(ctx.voice_state.songs.name(index-1)),color=0xFF5733)
